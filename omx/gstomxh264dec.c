...
static gboolean
gst_omx_h264_dec_is_new_frame (GstOMXVideoDec * decoder, guint8 * data,
    gsize size)
{
  guint8 type_byte;
  ...
  /* For now, just assume we have h264parse which guaranty AUD (type 9) */
  return (type_byte & 0x1f) == 9;
}
...
gst_omx_h264_dec_class_init (GstOMXH264DecClass * klass)
{
  ...
  videodec_class->is_new_frame =
      GST_DEBUG_FUNCPTR (gst_omx_h264_dec_is_new_frame);
